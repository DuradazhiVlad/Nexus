-- Fix Posts Structure to Work with user_profiles
...
-- 3. Ensure posts table has correct foreign key to user_profiles
-- (This should already be correct based on your current structure)
-- ALTER TABLE posts DROP CONSTRAINT IF EXISTS posts_user_id_fkey;
-- ALTER TABLE posts ADD CONSTRAINT posts_user_id_fkey 
-- FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE;

-- 4. Update RLS policies for posts to work with user_profiles
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.id = post_likes.user_id 
    AND user_profiles.auth_user_id = auth.uid()
  )
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.id = post_comments.user_id 
    AND user_profiles.auth_user_id = auth.uid()
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.id = albums.user_id 
    AND user_profiles.auth_user_id = auth.uid()
  )
);
...
CREATE TABLE public.media (
  ...
  CONSTRAINT media_user_id_fkey FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE,
...
USING (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.id = media.user_id 
    AND user_profiles.auth_user_id = auth.uid()
  )
);
