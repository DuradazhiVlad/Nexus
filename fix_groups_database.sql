/*
  # Виправлення проблем з таблицею groups та зв'язками з user_profiles
...
-- 1. Спочатку перевіримо поточну структуру таблиці groups
SELECT 
    column_name, 
    data_type, 
    is_nullable, 
...
-- 2. Додамо відсутні поля до таблиці groups якщо їх немає
...
-- 3. Видалимо старі foreign key constraints
...
-- 4. Додамо нові foreign key constraints що посилаються на user_profiles
...
-- 5. Створимо індекси для нових полів
...
-- 6. Перевіримо що всі foreign key constraints правильні
SELECT 
    tc.table_name, 
    tc.constraint_name, 
    tc.constraint_type,
...
-- 7. Перевіримо структуру таблиці groups після змін
SELECT 
    column_name, 
    data_type, 
    is_nullable, 
...
-- 8. Перевіримо структуру таблиці group_members
SELECT 
    column_name, 
    data_type, 
    is_nullable, 
